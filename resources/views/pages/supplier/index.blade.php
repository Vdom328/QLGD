...
@section('page_title_actions')
    <div class="col-12 d-flex flex-wrap align-items-center">
        ...
        <div class="d-flex col-md-6 col-12  mt-md-0 mt-2  justify-content-end">
            @include('components.btn-create-new', ['url' => route('supplier.create')])
        </div>
    </div>
@endsection
                ...
                <table class="table table-hover ">
                    ...
                    <tbody id="supplier_table">
                        @foreach ($suppliers as $supplier)
                            <tr data-id="{{ $supplier->id }}" class="cursor-pointer">
                                <td>{{ $supplier->code }}</td>
                                <td>{{ $supplier->name }}</td>
                                <td class="p-0">
                                    @foreach ( $supplier->supplier_managers as $key => $item  )
                                        <div class="col-12 d-flex align-items-center @if ($key !== count($supplier->supplier_managers) - 1) border-bottom @endif">
    ...
    <script>
        $(document).ready(function() {
            ...
            function filTer(column,direction)
            {
                ...
                window.location.href = "{{ route('supplier.index') }}?column=" + column + "&direction=" + direction + "&key=" + key + "&staff_id=" + staff_id + "&filter_me=" + filter_me + "&payment_terms=" + payment_terms;
            }

            // Add click event listener to each row
            $("#supplier_table tr").click(function() {
                const supplier_id = $(this).attr("data-id");
                if (supplier_id) {
                    window.location.href = "{{ route('supplier.getEdit') }}?id=" + supplier_id ;
                }
            });
