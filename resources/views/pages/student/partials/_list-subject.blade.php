@foreach ($data as $value)
    @if (isset($value->teacher_subject->subject))
        <tr>
            <td>{{ $value->teacher_subject->subject->credits_no }}</td>
            <td>{{ $value->teacher_subject->subject->name }} - Lớp: {{ $value->teacher_subject->class->name }} - GV: {{ $value->teacher_subject->teacher->profile->full_name }}</td>
