    ...
    <div method="post" action="{{ route('teacherSubject.create') }}"
        ...
        <div class="col-12 d-flex flex-wrap align-items-center">
            <div class="d-flex col-12  mt-md-0 mt-2  justify-content-end">
                <a class="btn-grey btn-modal cursor-pointer" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Thêm môn học cho
                    giảng viên</a>
            </div>
        </div>
    </div>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#create_subject', function(e) {
                ...
                $('input[name="class"]:checked').each(function() {
                    selectedSubjects.push($(this).val()); // Thêm giá trị của checkbox đã chọn vào mảng
                });
                $.ajax({
                    type: 'post',
                    data: {
