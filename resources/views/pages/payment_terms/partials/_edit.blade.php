    ...
    <div class="row col-12 d-flex flex-wrap align-items-center">
        ...
        <div class="col-xl-2 col-md-3 col-12  mb-md-0 mb-2">
            <select name="standard_type" id="standard_type" class="form-select">
                @for ($i = 1; $i < 10; $i++)
                    <option value="{{ $i }}" @if ($data->standard_type == $i) selected @endif @if (old('standard_type') == $i) selected @endif>
                        {{ $i }}</option>
                @endfor
            </select>
        </div>
        <div class="col-xl-1 col-md-3 col-12 mb-md-0 mb-2">
            <input type="number" class="form-control" name="standard_value"
                value="{{ old('standard_value', $data->standard_value) }}">
        </div>
        <div class="col-xl-2 col-md-3 col-12 mb-md-0 mb-2">
            <select name="standard_unit" id="standard_unit" name="standard_unit" class="form-select">
                @for ($j = 1; $j < 10; $j++)
                    <option value="{{ $j }}" @if ($data->standard_unit == $j) selected @endif @if (old('standard_unit') == $j) selected @endif> {{ $j }} 週間</option>
                @endfor
            </select>
        </div>
    </div>
